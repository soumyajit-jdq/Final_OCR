...
class ProcessingService:
    ...
    def build_canonical_payload(data: dict) -> str:
        ...
        payload = OrderedDict([
            ...
            ("examination_held_in", clean(data.get("examination_held_in"))),
            ("subjects", subjects)
        ])
    ...
    @staticmethod
    def post_process_extracted_marksheets(data: dict, ocr_text: str) -> dict:
        """
        Post-processes extracted marksheet data to:
        1. Strip trailing asterisks/footnotes from subject titles.
        2. Resolve fallback for 'examination_centre' if not extracted but present in OCR text.
        3. Clean up common OCR spelling errors.
        """
        if not isinstance(data, dict):
            return data

        marksheets = data.get("marksheets", [])
        if not isinstance(marksheets, list):
            return data
        ...
        for ms in marksheets:
            ...
            # Nullify not needed fields to exclude them from the response JSON
            # ms["earned_credits"] = None
            # ms["earned_grade_points"] = None
            
            # Clean / Fallback for examination_held_in
            exam_held = ms.get("examination_held_in")
            if not exam_held or str(exam_held).strip() == "" or str(exam_held).strip().lower() == "none":
                # Attempt to extract from OCR text via regex fallbacks
                held_match = re.search(r'Examination\s+held\s+in\s*[:\s-]+\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                if not held_match:
                    held_match = re.search(r'Examination\s+held\s+in\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                if held_match:
                    matched_val = held_match.group(1).strip()
                    matched_val = re.split(r'\t|\s{2,}', matched_val)[0].strip()
                    matched_val = matched_val.lstrip(":").strip()
                    ms["examination_held_in"] = matched_val
            else:
                ms["examination_held_in"] = str(exam_held).lstrip(":").strip()
            
            # Clean subjects
            for sub in ms.get("subjects", []) or []:
                if isinstance(sub, dict):
                    # Fix comma/period issue in subject codes (e.g., "AG. ENTO, 3.3" → "AG. ENTO. 3.3")
                    if "code" in sub and sub["code"]:
                        code = str(sub["code"])
                        # Replace comma with period when followed by space and number (common OCR error)
                        code = re.sub(r',\s+(\d+)', r'. \1', code)
                        sub["code"] = code
                    
                    if "title" in sub and sub["title"]:
                        title = str(sub["title"])
                        # Remove trailing asterisks and footnote symbols, plus extra spaces
                        title = re.sub(r'[\*\@\#\†\‡\§\¶\s]+$', '', title).strip()
                        sub["title"] = title
            
            # Check if examination_centre is missing
            exam_centre = ms.get("examination_centre")
            if not exam_centre or str(exam_centre).strip() == "" or str(exam_centre).strip().lower() == "none":
                # Attempt to extract from OCR text via regex fallbacks
                poly_match = re.search(r'Name\s+of\s+Polytechnic\s*:\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                college_match = re.search(r'Name\s+of\s+College\s*:\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                inst_match = re.search(r'Name\s+of\s+Institution\s*:\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                centre_match = re.search(r'Examination\s+Centre\s*([^\n\t\r]+)', ocr_text, re.IGNORECASE)
                
                matched_val = ""
                if poly_match:
                    matched_val = poly_match.group(1).strip()
                elif college_match:
                    matched_val = college_match.group(1).strip()
                elif inst_match:
                    matched_val = inst_match.group(1).strip()
                elif centre_match:
                    matched_val = centre_match.group(1).strip()
                    
                if matched_val:
                    # Clean up multiple tabs/spaces or keywords
                    matched_val = re.split(r'\t|\s{2,}', matched_val)[0].strip()
                    # Remove any leading/trailing colons or symbols
                    matched_val = matched_val.lstrip(":").strip()
                    ms["examination_centre"] = matched_val

            # Clean obvious typos in examination_centre
            if ms.get("examination_centre"):
                val = str(ms["examination_centre"])
                # Typo correction: AGRICUI TURE -> AGRICULTURE
                val = re.sub(r'\bAGRICUI\s*TURE\b', 'AGRICULTURE', val, flags=re.IGNORECASE)
                # Remove trailing/leading colons
                val = val.lstrip(":").strip()
                ms["examination_centre"] = val
...
#### EXTRACTION RULES ####
- **Examination Centre Mapping**: If the document contains fields like 'Name of College', 'Name of Polytechnic', or 'Name of Institution', map its value to the 'examination_centre' field. Correct obvious OCR typos in this name (e.g., 'POLYTECHNIC IN AGRICUI TURE' -> 'POLYTECHNIC IN AGRICULTURE').
- **Clean Subject Titles**: Remove trailing asterisks ('*'), footnote symbols, or other non-alphanumeric footnote markers from subject titles. For example, 'NSS/NCC/PHYSICAL EDUCATION AND YOGA PRACTICES (NON-GRADIAL)*' must be extracted as 'NSS/NCC/PHYSICAL EDUCATION AND YOGA PRACTICES (NON-GRADIAL)'.
- **Detailed Marksheet Columns**: If the marksheet has detailed columns for Theory/Practical CCE and SEE (both Min/Max and Obtained), extract them verbatim into the corresponding fields (e.g. theory_cce_min_max, theory_cce_obtained, theory_see_min_max, theory_see_obtained, practical_cce_min_max, practical_cce_obtained, practical_see_min_max, practical_see_obtained, total_min_max, total_obtained) for each subject.
...
- **Course Code**: "PL. PATH. 1.4" must be extracted as "PL. PATH. 1.1".
...
"""
        ...
        if GEMINI_API_KEY:
            try:
                ...
                return ProcessingService.post_process_extracted_marksheets(result, ocr_text)
            except Exception as e:
        ...
        if CEREBRAS_API_KEY:
            ...
            if cerebras_result:
                return ProcessingService.post_process_extracted_marksheets(cerebras_result, ocr_text)
